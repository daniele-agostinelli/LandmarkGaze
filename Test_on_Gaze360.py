...
class GazeBenchmarkFromCSV:
    def __init__(self, config, model_type='normalized', output_csv_path=None, summary_csv_path=None):
        ...
        if self.model_type == 'normalized_siamese':
            ...
            # Default to standard normalized
        ...
        # Cache the indices expected by the model
        ...
        self.fieldnames = [
            "recording", "person_id", "frame_index", "split_id",
    ...
    def run_benchmark(self, input_csv_path):
        ...
        try:
            with open(input_csv_path, 'r') as f:
                # The generator script uses ';' as delimiter
                reader = csv.DictReader(f, delimiter=';')
                
                # Verify headers exist for all required landmarks
                headers = reader.fieldnames
                for idx in self.landmark_indices:
                    if f"{idx}_x" not in headers or f"{idx}_y" not in headers:
                        logger.error(f"Missing columns for landmark {idx} in Input CSV. Cannot proceed.")
                        return

                for row_idx, row in enumerate(reader):
                    try:
                        # 1. Parse Ground Truth Gaze (Normalized)
                        gt_gaze = np.array([
                            float(row['gaze_x']),
                            float(row['gaze_y']),
                            float(row['gaze_z'])
                        ...
                        # 3. Inference
                        # We use the subroutine exposed by the estimator
                        ...
                        if not np.isnan(error):
                            ...
                            if writer:
                                out_row = {
                                    ...
                                    "person_id": row.get('subject', ''), # Generator uses 'subject' for pid
                                    "frame_index": row.get('frame', ''),
                                    "split_id": row.get('split', ''),
                                    "gt_gaze_x": gt_gaze[0],
                                    "gt_gaze_y": gt_gaze[1],
                                    "gt_gaze_z": gt_gaze[2],
...
if __name__ == "__main__":
    ...
    # --- CONFIGURATION ---
    # Path to the specific CSV file you want to test (e.g., Test set)
    CSV_PATH_CROSS_TEST = './datasets/Gaze360/gaze360_normalized_ALL_USED.csv'
    CSV_PATH_WITHIN_TEST = './datasets/Gaze360/gaze360_normalized_TEST.csv'
    ...
    for model_def in models_to_benchmark:
        ...
        # Prepare Output Path
        output_csv = f'./models/stats/gaze360/CSV files/benchmark_results_csv_{model_def["name"]}.csv'
        summary_file = './models/stats/summary_benchmark_gaze360.csv'

        try:
            benchmark = GazeBenchmarkFromCSV(
                config, 
